...
template <class T>
FarfalleLike<T>::FarfalleLike(IteratedTransformation<T> &p_n, IteratedTransformation<T> &p_r, RollingFunction<T> &roll,const T IV):
	p_n(p_n), 
	p_r(p_r), 
	roll(roll),
...
template <class T>
tuple<Vec<T>, T> FarfalleLike<T>::encrypt(const Vec<T> &MK, const T &nonce, const Vec<T> &Mseq) const
{
...
template <class T>
Vec<T> FarfalleLike<T>::decrypt(const Vec<T> &MK, const T &nonce, const Vec<T> &Cseq, const T &tag) const
{
	if (Cseq.length() % 2 != 0) {
		throw "Expecting an even number of messages.";
	}
	
	long number_of_pairs = Cseq.length()/2;
	...
	for (long i = 0; i < number_of_pairs; i++) {
		...
		p_r(outer_branches);
		// Encrypt the plaintexts
		Mseq[2*i] = Cseq[2*i] - outer_branches[0];
		Mseq[2*i+1] = Cseq[2*i+1] - outer_branches[1];
		// Compute part of MAC 
		accum += Cseq[2*i];
		accum *= working_keys[0];
		accum += Cseq[2*i+1];
	...
	// Compute tag
	p_r(inner_upper_branches);
	T computed_tag = inner_upper_branches[0] + accum;
	if(computed_tag != tag) {
		throw "Tag mismatch.";
	}
