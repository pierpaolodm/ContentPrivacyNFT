...
contract PrivacyPreservingNFT is ERC721URIStorage, Ownable{
    ...
    constructor(address _poseidonLib, address _secp256k1) ERC721("PrivacyPreservingNFT","PPN") Ownable(msg.sender) {
        token_ID = 0;
        poseidon = PoseidonT4(_poseidonLib);
    ...
    */
    ...
    function mint_token(address owner, string memory token_URI, uint256[2] memory owner_publickey,
        ...
        require(secp.isOnCurve(owner_publickey[0], owner_publickey[1]), "public key not on curve");
    ...
    */
    function place_bid(uint256 tokenId, uint256[2] memory bidder_publickey) public solvedNFT(tokenId) payable{
        require(secp.isOnCurve(bidder_publickey[0], bidder_publickey[1]), "public key not on curve");
    ...
    */
    function deny_transfer(uint256 tokenId, uint256 privatekey) public onlyNFTOwner(tokenId){
        ...
        (uint256 x, uint256 y) = secp.derivePubKey(privatekey);
        ...
        (uint256 DH_x, uint256 DH_y) = secp.pointMul(publickey[0], publickey[1], privatekey);
        ...
        // To burn the token, the commitments must be the same. Otherwise abort... seller is right
